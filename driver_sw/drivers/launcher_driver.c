...
#define USB_LAUNCHER_VENDOR_ID	LAUNCHER_VENDOR_ID
#define USB_LAUNCHER_PRODUCT_ID	LAUNCHER_PRODUCT_ID
			  ...
			  size_t count, loff_t *ppos)
{
	...
	// user defined buffer for ctrl message
	char *ctrl_buf = NULL;
	size_t ctrl_buf_size = LAUNCHER_CTRL_BUFFER_SIZE; // idk what size to go with here, tbh
	...
	// TODO: I think here is where we should set the retval to contain the usb launcher command info
	// initialize a URB and assign retval to it
	ctrl_buf = (char*)kzalloc( ctrl_buf_size * sizeof( char ), GFP_KERNEL ); // allocate space for the buffer
	ctrl_buf[0] = LAUNCHER_CTRL_COMMAND_PREFIX;
	ctrl_buf[1] = buf[0];
	// fill the ctrl message buffer with the right stuff


	// We want to send a control message, not a bulk message like the skeleton
	// you can find instructions on the usb_control_msg here: https://manpages.debian.org/jessie-backports/linux-manual-4.8/usb_control_msg.9.en.html
	retval = usb_control_msg(
		...
		ctrl_buf,
		ctrl_buf_size,
		500 // wait 500ms for the message to complete before timing out
		);
	mutex_unlock(&dev->io_mutex);
...
// TODO: change the info in this URB to work for our missile launcher
	...
	// printk("freeing urb\n");
	usb_free_urb(urb);
	// printk("freeing ctrl_buf\n");
	kfree( ctrl_buf );
	// printk("freeing buf\n");
	// kfree( buf );
	up(&dev->limit_sem);
	// printk("running up cmd\n");
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR( "Team S2-1" );
MODULE_DESCRIPTION( "USB missile launcher driver" );